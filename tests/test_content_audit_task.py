...
@mock.patch("ubi_manifest.worker.tasks.content_audit.get_content_configs")
def test_pipeline(get_content_configs, pulp, caplog):
    caplog.set_level(logging.DEBUG, logger="ubi_manifest.worker.tasks.auditing")
    get_content_configs.return_value = [{"source": "foo"}]
...
@mock.patch("ubi_manifest.worker.tasks.content_audit.get_content_configs")
def test_pipeline_with_invalid_repo(get_content_configs, mock_loader, pulp):
    get_content_configs.return_value = [{"source": "foo", "branch_prefix": "bar"}]
