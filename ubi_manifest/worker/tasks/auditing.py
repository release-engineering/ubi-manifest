...
class UbiReposBundle(TypedDict):
    """
    Representation of batched repos, bundled by type.
    """

    bin_repo: YumRepository
...
class ContentProcessor:
    ...
    Attributes:
        ...
        out_repo_bundle (UbiReposBundle): A bundle of bin, debug and source out repos.
    ...
    def process_and_audit_bundle(self) -> None:
        ...
        for repo_type in ["bin_repo", "source_repo", "debug_repo"]:
            out_repo = self.out_repo_bundle[repo_type]  # type: ignore
            in_repos = self.in_repos_map.get(f"{repo_type}s", [])
    ...
    def _fetch_in_repos_contents(self, in_repos: List[YumRepository]) -> None:
        ...
        default_arch = self.out_repo_bundle["bin_repo"].arch
        _LOG.info("default arch: %s\n", default_arch)
        for pkg in self.nonmodular_auditor.whitelist:  # type: ignore
            if not any(name == pkg for name, _ in names_arch):
                names_arch.add((pkg, default_arch))
        rpm_units_criteria = create_or_criteria(fields, list(names_arch))
        all_rpm_units: set[UbiUnit] = search_rpms(
