...
class NonModularAuditor:
    ...
    def validate_versions(self) -> None:
        ...
        def log_warning(warn_dict: dict[str, dict[str, str]]) -> None:
            _LOG.warning(
                "[%s] UBI rpm '%s' is outdated (current: (%s, %s, %s), latest: (%s, %s, %s))",
                self.out_repo_id,
                warn_dict["name"],
                warn_dict["out"]["epoch"],
                warn_dict["out"]["version"],
                warn_dict["out"]["release"],
                warn_dict["in"]["epoch"],
                warn_dict["in"]["version"],
                warn_dict["in"]["release"],
            )
        ...
        for out_unit in self.out_units:
            for in_unit in self.in_units:
                if (out_unit.name, out_unit.arch) == (in_unit.name, in_unit.arch):
                    in_evr = {
                        "epoch": in_unit.epoch,
                        "version": in_unit.version,
                        "release": in_unit.release,
                    }
                    out_evr = {
                        "epoch": out_unit.epoch,
                        "version": out_unit.version,
                        "release": out_unit.release,
                    }

                    if RELATION_CMP_MAP["LT"](
                        (out_evr["epoch"], out_evr["version"], out_evr["release"]),
                        (in_evr["epoch"], in_evr["version"], in_evr["release"]),
                    ):  # type: ignore
                        log_warning(
                            {
                                "name": out_unit.name,
...
class ContentProcessor:
    ...
    def process_and_audit(self) -> None:
        ...
        if self.is_out_modular:
            ...
            _LOG.info("Modular filenames: %s\n", self.all_modular_filenames)
        else:
